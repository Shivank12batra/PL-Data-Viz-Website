...
const Home = ({ navOpen, setNavOpen }) => {
  ...
  useEffect(() => {
    ...
    const bgTimer = setInterval(() => {
      backgroundObj.id === bgmImages[bgmImages.length - 1].id
        ? setBackgroundObj(bgmImages[0])
        : setBackgroundObj(bgmImages[backgroundObj.id]);
  ...
  return (
    <div className="relative h-screen z-0">
      ...
      <animated.h1
        ...
        key={backgroundObj.id}
        className={`${
          headerVisible ? "" : "opacity-0"
        } absolute top-16 left-1/2 transform -translate-x-1/2`}
      >
        <span className="text-white text-3xl font-bold inline-block bg-black bg-opacity-70 px-6 py-4 text-center sm:text-5xl sm:px-20">
          {backgroundObj.header}
        </span>
      </animated.h1>
      ...
      <animated.div className="absolute top-2/3 left-1/3 transform -translate-x-1/2 -translate-y-1/2 ml-8">
        {backgroundObj.id !== 1 ? (
          <animated.button
            ...
            className={`${
              buttonVisible ? "" : "opacity-0"
            } group text-white font-bold text-sm bg-black bg-opacity-70 px-4 py-2 my-2 hover:bg-red-500 transition-all duration-200`}
        ...
        <animated.p
          ...
          } text-white text-sm bg-black bg-opacity-70 max-w-md px-2 py-2 sm:text-lg z-0`}
        >
          {backgroundObj.content}
        </animated.p>
      </animated.div>
      ...
      <animated.div className="absolute right-0 top-1/2 transform -translate-y-1/2 sm:right-6">
        <animated.button
          ...
          className={`${
            arrowVisible ? "" : "hidden"
          } bg-gray-500 bg-opacity-50 p-1 hover:bg-gray-300 transition-colors duration-300 sm:p-2`}
        >
          <FaAngleRight
            size={22}
            onClick={() => {
        ...
        >
          <FaAngleLeft
            ...
            onClick={() => {
              ...
              setButtonVisible(false);
              backgroundObj.id === 1
                ? setBackgroundObj(bgmImages[bgmImages.length - 1])
                : setBackgroundObj(bgmImages[backgroundObj.id - 2]);
            }}
          />
        </animated.button>
      </animated.div>
