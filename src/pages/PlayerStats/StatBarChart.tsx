...
const StatBarChart = () => {
  ...
  const transformStat =
    stat === "Pass%20Completion%20%"
      ? "Pass Completion %"
      : (stat as unknown as TPlayerStat); // TODO: hacky fix, need to have a proper solution for this later

  const { data, isLoading, error } = fetchTopPlayersData({
    team,
    stat: transformStat,
  });
  ...
  const barChart = () => {
    ...
    const chartData: TChartData[] = data.map((d) => ({
      name: d["name"],
      value:
    ...
    function handleMouseOver(
      ...
      d3.select(event.currentTarget).attr("fill", "rgba(0, 0, 0, 0.2)");
    ...
    function handleMouseOut(
      ...
      d3.select(event.currentTarget).attr("fill", teamColorMapping[team].color);
    ...
    const yScale = d3
      .scaleLinear()
      .domain([0, d3.max(chartData, (d) => d.value + 5) as number])
      .range([innerHeight, 0]);
    ...
    svg
      ...
      .selectAll("text")
      .text((label: any) => {
        //TODO: inspect into this issue of why its giving error on giving string type
    ...
    svg
      ...
      .attr("x1", 0)
      .attr("x2", innerWidth)
      .attr("y1", yScale(medianValue as number))
      .attr("y2", yScale(medianValue as number))
    ...
    svg
      ...
      .attr("x", innerWidth)
      .attr("y", yScale(medianValue as number) - 10) // Slightly above the line (adjust 10 for the desired offset)
      .attr("fill", "white")
