...
const PlayerEventMap = ({
  ...
  const eventMap = () => {
    ...
    const dotSizeScale = d3
      .scaleLinear()
      .domain([0, 0.1, 0.2, 0.5, 0.75, 1])
      .range([0.5, 1, 1.5, 2, 2.5, 3]);
    ...
    if (mapType === "Other Events") {
      if (eventType === "Pass") {
        ...
        const ballRecoveries = pitchSvg
          ...
          .attr(
            "transform",
            (d) => `translate(${d.x * 1.05}, ${(100 - d.y) * 0.68})`
          );
    ...
    const guideDotsData = [{ xG: 0.1 }, { xG: 0.2 }, { xG: 0.6 }, { xG: 0.8 }];

    // Define the position of the guide dots along the x-axis (horizontally)
    const guideDotXPositions = [68 * 0.36, 68 * 0.42, 68 * 0.5, 68 * 0.6];
    ...
    guideDotsGroup
      ...
      .attr("fill", (_, i) =>
        i === 3 ? "lightgreen" : teamColorMapping[team].color
      )
      .attr("stroke", "black")
      .attr("stroke-width", 0.5);
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-center text-white m-2">
        <button
          className={`${mapType === "Shotmap" && team === "Tottenham" ? "text-black" : null} py-2 px-8 border-2 cursor-pointer transition-colors duration-100`}
          style={{
            borderColor: `${teamColorMapping[team].color}`,
            backgroundColor: `${mapType === "Shotmap" ? teamColorMapping[team].color : ""}`,
          }}
        ...
        <button
          className={`${mapType === "Other Events" && team === "Tottenham" ? "text-black" : null} py-2 px-8 border-2 cursor-pointer transition-colors duration-100`}
          style={{
            borderColor: `${teamColorMapping[team].color}`,
            backgroundColor: `${mapType === "Other Events" ? teamColorMapping[team].color : ""}`,
          }}
      ...
      {mapType === "Other Events" ? (
        <div className="flex justify-center text-white m-2">
          <button
            className={`${eventType === "Pass" && team === "Tottenham" ? "text-black" : null} py-2 px-8 border-2 cursor-pointer transition-colors duration-100`}
            style={{
              borderColor: `${teamColorMapping[team].color}`,
              backgroundColor: `${eventType === "Pass" ? teamColorMapping[team].color : ""}`,
            }}
          ...
          <button
            className={`${eventType === "BallRecovery" && team === "Tottenham" ? "text-black" : null} py-2 px-8 border-2 cursor-pointer transition-colors duration-100`}
            style={{
              borderColor: `${teamColorMapping[team].color}`,
              backgroundColor: `${eventType === "BallRecovery" ? teamColorMapping[team].color : ""}`,
            }}
      ...
      {mapType === "Other Events" ? (
        <div className="flex justify-center text-white m-2">
          <button
            className={`${eventOutcome === "Successful" && team === "Tottenham" ? "text-black" : null} py-2 px-8 border-2 cursor-pointer transition-colors duration-100`}
            style={{
              borderColor: `${teamColorMapping[team].color}`,
              backgroundColor: `${eventOutcome === "Successful" ? teamColorMapping[team].color : ""}`,
            }}
          ...
          <button
            className={`${eventOutcome === "Unsuccessful" && team === "Tottenham" ? "text-black" : null} py-2 px-8 border-2 cursor-pointer transition-colors duration-100`}
            style={{
              borderColor: `${teamColorMapping[team].color}`,
              backgroundColor: `${eventOutcome === "Unsuccessful" ? teamColorMapping[team].color : ""}`,
            }}
      ...
      {mapType === "Other Events" && data ? (
        <div className="flex justify-center text-white mt-4">
          {data.length} {eventOutcome}{" "}
          {eventType === "Pass" ? "Passes" : "Ball Recoveries"}
        </div>
      ) : null}
