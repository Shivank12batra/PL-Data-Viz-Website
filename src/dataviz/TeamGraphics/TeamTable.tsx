...
const TeamTable = () => {
  ...
  const isMatchingTeam = (squad: TTeam) => squad === team;

  // custom sort for the 'form' column
  const formSort = (rowA: Row<TTeamTable>, rowB: Row<TTeamTable>) => {
    const formA = rowA.original.Form;
    const formB = rowB.original.Form;
    const pointsA = formA.split("").reduce((total: number, value: string) => {
      if (value === "W") {
        total += 3;
      } else if (value === "D") {
        total += 1;
    ...
    const pointsB = formB.split("").reduce((total: number, value: string) => {
      if (value === "W") {
        total += 3;
      } else if (value === "D") {
        total += 1;
  ...
  const xGDSort = (rowA: Row<TTeamTable>, rowB: Row<TTeamTable>) => {
    const numA = rowA.original.xGD;
    const numB = rowB.original.xGD;
    if (numA > numB) {
      return 1;
    }
    if (numA < numB) {
  ...
  const columns = React.useMemo(() => {
    const columnHeaders = Object.keys(data[0]) as (keyof TTeamTable)[];
  ...
  return (
    <div className="flex flex-col items-center border-transparent border-2 mt-32">
      ...
      <div className="overflow-x-auto w-full sm:w-4/5">
        <table
          {...getTableProps()}
          className="w-full border-collapse border border-gray-500 rounded-lg shadow-lg mt-8 table-auto"
        >
