...
const TeamShotMap = ({ homeTeam, awayTeam }: ITeamShotMapProps) => {
  const { team } = useAuth();
  ...
  const { data, isLoading, error } = fetchShotMapData({
    team,
    homeTeam,
    awayTeam,
  });
  ...
  const shotMap = () => {
    ...
    const shotsDots = pitchSvg
      ...
      .attr(
        "transform",
        (d: TShotData) =>
          `translate(${Number(d.X) * 105}, ${(1 - Number(d.Y)) * 68})`
      );

    shotsDots
      .append("circle")
      .attr("class", "shot-dot")
      .attr("r", (d: TShotData) => dotSizeScale(Number(d.xG)))
      .attr("fill", (d) =>
